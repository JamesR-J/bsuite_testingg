...
class ActorCritic(base.Agent):
    ...
    def __init__(
        ...
        def loss(trajectory: sequence.Trajectory, gae, targets) -> jnp.ndarray:
            """"Actor-critic loss."""
            ...
            traj_batch_log_probs = distrax.Softmax(trajectory.logits).log_prob(trajectory.actions)

            # CALCULATE VALUE LOSS
            clip_eps = 1.0  # TODO sort this out
            value_pred_clipped = trajectory.values + (values - trajectory.values).clip(-clip_eps, clip_eps)
            ...
            total_loss = loss_actor + value_loss - entropy  # TODO check this is correct
        ...
        def sgd_step(state: TrainingState,
                     ...
                     gae,
                     targets) -> TrainingState:
            """Does a step of SGD over a trajectory."""
            (total_loss, (loss_actor, value_loss, entropy)), gradients = jax.value_and_grad(loss_fn, has_aux=True)(state.params, trajectory,
                                                                                      gae,
                                                                                      targets)
            ...
            return (TrainingState(params=new_params, opt_state=new_opt_state, tau_params=new_tau_params,
                                 tau_opt_state=new_tau_opt_state), loss_actor, value_loss, tau, tau_loss_val)
    ...
    def update(self,
        ...
        if self._buffer.full() or new_timestep.last():
            ...
            gae_lambda = 1.0  # TODO sort this oot
            _, last_val = self._forward(self._state.params, trajectory.observations[-1])  # TODO is this correct?

            tau = jnp.exp(self._state.tau_params)

            def _calculate_gae(trajectory, last_val):
                def _get_advantages(gae_and_next_value, transition):
                    gae, next_value = gae_and_next_value
                    discount, value, reward = (
                        transition.discounts,
                        transition.value,
                        transition.rewards + (state_action_reward_noise / (2 * tau))  # TODO ensure get tau correctly
                    )
                    delta = reward + discount * next_value * discount - value
                    gae = (delta + discount * gae_lambda * discount * gae)
                ...
                return advantages, advantages + trajectory.values

            advantages, targets = _calculate_gae(trajectory, last_val)

            # UPDATE NETWORK
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    trajectory, state_action_reward_noise, advantages, targets = batch_info
                    self._state, actor_loss, value_loss, tau, tau_loss_val = self._sgd_step(self._state, trajectory, state_action_reward_noise, advantages, targets)

                    return (actor_loss, value_loss, tau, tau_loss_val)
