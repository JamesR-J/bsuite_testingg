...
class Buffer:
  ...
  def __init__(
    ...
    self._actions = np.zeros(
        shape=(max_sequence_length + 1, *action_spec.shape),
        dtype=action_spec.dtype)
    self._logits = np.zeros(
        shape=(max_sequence_length + 1, 2),  # TODO change this hardcoding
        dtype=obs_spec.dtype)
    self._rewards = np.zeros(max_sequence_length + 1, dtype=np.float32)
    self._discounts = np.zeros(max_sequence_length + 1, dtype=np.float32)
    self._step = np.zeros(max_sequence_length + 1, dtype=np.float32)
    self._mask = np.zeros((max_sequence_length + 1, num_ensemble), dtype=np.float32)
    self._noise = np.zeros((max_sequence_length + 1, num_ensemble), dtype=np.float32)
  ...
  def append(
    ...
    if self._needs_reset:
      ...
      self._rewards[self._t] = 0.0
      self._discounts[self._t] = 0.0
      self._step[self._t] = timestep.step_type
    ...
    self._rewards[self._t + 1] = new_timestep.reward
    self._discounts[self._t + 1] = new_timestep.discount
    self._step[self._t + 1] = new_timestep.step_type
  ...
  def drain(self) -> Trajectory:
    ...
    trajectory = Trajectory(
        ...
        self._actions[:self._t + 1],
        self._logits[:self._t + 1],
        self._rewards[:self._t + 1],
        self._discounts[:self._t + 1],
        self._step[:self._t + 1],
        self._mask[:self._t + 1],
        self._noise[:self._t + 1],
    )
